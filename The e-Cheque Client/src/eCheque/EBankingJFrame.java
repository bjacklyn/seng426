        ...
        if(hostName.length()!=0){
            ...
            if(selectChequeFlag){             
                ...
                if(jCBDeposit.isSelected()){
                    Runnable client = new EchequeClient(EChequeUtil.SERVER_PORT,BankMode.DEPOSIT,hostName,registerData,depositCheque);
                    Thread clientThread = new Thread(client);
                    clientThread.start();
                }
                else if(jCBCancel.isSelected())
                {
                    Runnable client = new EchequeClient(EChequeUtil.SERVER_PORT,BankMode.CANCEL,hostName,registerData,depositCheque);
                    Thread clientThread = new Thread(client);
                    clientThread.start();
                }
                else{
